...
DATE CREATED:  @Time : 2018/3/29 下午4:42
...
CHANGE:  : 
MODIFIED: : @Time : 2018/3/29 下午4:42
"""
...
class RNNModel(object):
    def __init__(self, is_test, hidden_size, rnn_layers, batch_size, seq_length, vocab_size, embedding_size,
        ...
        inputs = tf.nn.embedding_lookup(self.embedding_size, self.input_data)

        sliced_inputs = [tf.squeeze(input_, [1]) for input_ in tf.split(axis=1, value=inputs,
        ...
        soft_max_w = tf.get_variable('soft_max_w', [hidden_size, vocab_size])
        soft_max_b = tf.get_variable('soft_max_b', [vocab_size])
        self.logits = tf.matmul(flat_outputs, soft_max_w) + soft_max_b
        ...
        if is_test == 'false':
            ...
            grads, _ = tf.clip_by_global_norm(tf.gradients(mean_loss, t_vars), max_grad_norm)
    ...
    def train(self, session, train_size, train_batches):
        ...
        for step in range(epoch_size):
            ...
            average_loss, state, _, global_step, learning_rate = session.run(ops, feed_dict)
        logging.info('average loss: %.3f, speed: %.0f chars per sec', average_loss,
                     (step + 1) * self.batch_size * self.seq_length / (time.time() - start_time))
...
def create_tuple_placeholders_with_default(inputs, shape):
    if isinstance(shape, int):
        ...
        sub_placeholders = [create_tuple_placeholders_with_default(sub_inputs, sub_shape)
