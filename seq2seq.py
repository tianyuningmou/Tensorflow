...
DATE CREATED:  @Time : 2018/3/28 下午5:11
...
CHANGE:  : 
MODIFIED: : @Time : 2018/3/28 下午5:11
"""
...
# 随机序列生成器
# length_from和length_to表示序列的长度范围从多少到多少
# vocab_lower和vocab_upper表示生成的序列值的范围从多少到多少
# batch_size 是批的数量
...
def make_batch(inputs, max_sequence_length=None):
    ...
    if max_sequence_length is None:
        max_sequence_length = len(sequence_lengths)
...
with train_graph.as_default():
    # 创建三个占位符，分别为encoder的input占位符、decoder的input占位符和decoder的target占位符
    ...
    # encoder_hidden_units 为LSTM隐层数量
